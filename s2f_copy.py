...
def infoFromCoinMetrics(period, proxies):
    """Compute Stock-to-Flow ratio and price for next five years."""
    result = []

    for i in range(5):  # for next 5 years
        date_to_check = datetime.date.today() + datetime.timedelta(days=365 * i)  
        dateYesterday = date_to_check - datetime.timedelta(days=1)
        ...
        result.append((date_to_check, stock_to_flow_ratio, stock_to_flow_usd))
...
def s2f(args):
    ...
    period = 463
    stock_to_flow_data = infoFromCoinMetrics(period, proxies)

    # Save data to a txt file
    with open('stock_to_flow_data.txt', 'w') as file:
        file.write('Date,Stock-to-Flow Ratio,Stock-to-Flow Price\n')
        for data in stock_to_flow_data:
            date, stock_to_flow_ratio, stock_to_flow_usd = data
            file.write(f'{date},{stock_to_flow_ratio},{stock_to_flow_usd}\n')
    (
        usd_price,
        circulating,
