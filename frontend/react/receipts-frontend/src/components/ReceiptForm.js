...
function ReceiptForm({ onSubmit }) {
  ...
  const handleSubmit = async (event) => {
    ...
    const cleanedItems = items.filter(
      (item) => item.item_name || item.price || item.category,
    );
    ...
    if (store === "")
      missingStoreField = "Please provide the name of the store.";
    ...
    if (missingStoreField || missingItemFields.length > 0) {
      ...
      if (missingItemFields.length > 0) {
        alertMessage +=
          "All items must have a " + missingItemFields.join(" and ") + "!";
      }
    ...
    let totalSum = cleanedItems.reduce(
      (total, item) => total + parseFloat(item.price || 0),
      0,
    );
